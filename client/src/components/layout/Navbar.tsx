  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-white/5 bg-background/80 backdrop-blur-xl navbar-glow">
      ...
      <div className="absolute top-0 left-0 right-0 h-[1px]" style={{
        background: 'linear-gradient(90deg, transparent, rgba(0,229,118,0.8), rgba(0,184,212,0.8), rgba(190,0,255,0.8), rgba(255,0,110,0.8), rgba(255,230,0,0.8), transparent)',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center space-x-2 group cursor-pointer">
            <div className="relative w-9 h-9 rounded-xl flex items-center justify-center shadow-lg transition-all duration-300 group-hover:scale-110"
              style={{
                background: 'linear-gradient(135deg, #00E576, #00B8D4, #BE00FF)',
                boxShadow: '0 0 15px rgba(0,229,118,0.4), 0 0 30px rgba(0,184,212,0.2)',
              }}>
              <Activity className="w-5 h-5 text-black" />
              {/* pulse ring */}
              <div className="absolute inset-0 rounded-xl border border-primary/30 animate-ping" style={{ animationDuration: '2s' }} />
            </div>
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[9px] uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-1">
                <Zap className="w-2.5 h-2.5 text-primary" />Engine Live
              </span>
            </div>
          </Link>
          ...
          {user && (
            <div className="flex items-center space-x-4">
              <div className="hidden md:flex items-center space-x-3 px-4 py-1.5 rounded-full border electric-border"
                style={{ background: 'rgba(255,255,255,0.03)' }}>
                <div className="relative">
                  {user.profileImageUrl ? (
                    <img src={user.profileImageUrl} alt="Profile" className="w-6 h-6 rounded-full ring-1 ring-primary/40" />
                  ) : (
